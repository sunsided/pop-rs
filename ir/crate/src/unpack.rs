// @generated by pop_lifter — DO NOT EDIT.
// POP source segment `UNPACK` (21 routines),
// lifted to free functions over the shared `Cpu`.

use crate::cpu::Cpu;
use crate::sym;

pub fn SNGEXPAND(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.mem[sym::RAMRDaux] = cpu.reg.a;
                cpu.mem[sym::RAMWRTmain] = cpu.reg.a;
                cpu.mem[sym::PAC + 1] = cpu.reg.a;
                cpu.reg.a = 0x20;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PIC + 1] = cpu.reg.a;
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PAC] = cpu.reg.a;
                cpu.mem[sym::PIC] = cpu.reg.a;
                cpu.reg.a = 0xfe;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V8] = cpu.reg.a;
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::VA] = cpu.reg.a;
                cpu.reg.y = 0x27;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                pc = 1;
            }
            1 => {
                cpu.reg.a = 0x78;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V2] = cpu.reg.a;
                cpu.reg.a = 0x20;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V3] = cpu.reg.a;
                pc = 2;
            }
            2 => {
                cpu.reg.a = cpu.mem[sym::V2];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x28_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V2] = cpu.reg.a;
                if cpu.flags.c {
                    pc = 4;
                } else {
                    pc = 3;
                }
            }
            3 => {
                let _v = cpu.mem[sym::V3].wrapping_sub(1);
                cpu.mem[sym::V3] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 4;
            }
            4 => {
                cpu.reg.a = cpu.mem[sym::V2];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V4] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::V3];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = false;
                let _r = (cpu.reg.a as u16) + (0x04) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V5] = cpu.reg.a;
                pc = 5;
            }
            5 => {
                cpu.reg.a = cpu.mem[sym::V4];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x80_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V4] = cpu.reg.a;
                if cpu.flags.c {
                    pc = 7;
                } else {
                    pc = 6;
                }
            }
            6 => {
                let _v = cpu.mem[sym::V5].wrapping_sub(1);
                cpu.mem[sym::V5] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 7;
            }
            7 => {
                cpu.reg.a = cpu.mem[sym::V4];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PIC] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::V5];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = false;
                let _r = (cpu.reg.a as u16) + (0x20) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PIC + 1] = cpu.reg.a;
                pc = 8;
            }
            8 => {
                cpu.reg.a = cpu.mem[sym::PIC + 1];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x04_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PIC + 1] = cpu.reg.a;
                cpu.flags.c = false;
                if !cpu.flags.c {
                    pc = 15;
                } else {
                    pc = 9;
                }
            }
            9 => {
                cpu.reg.a = cpu.mem[sym::PIC + 1];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = cpu.mem[sym::V5];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != cpu.mem[sym::V5] {
                    pc = 8;
                } else {
                    pc = 10;
                }
            }
            10 => {
                cpu.reg.a = cpu.mem[sym::V4];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = cpu.mem[sym::V2];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != cpu.mem[sym::V2] {
                    pc = 5;
                } else {
                    pc = 11;
                }
            }
            11 => {
                cpu.reg.a = cpu.mem[sym::V5];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = cpu.mem[sym::V3];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != cpu.mem[sym::V3] {
                    pc = 5;
                } else {
                    pc = 12;
                }
            }
            12 => {
                cpu.reg.a = cpu.mem[sym::V2];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    pc = 2;
                } else {
                    pc = 13;
                }
            }
            13 => {
                let _v = cpu.reg.y.wrapping_sub(1);
                cpu.reg.y = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if (cpu.reg.y as i8) >= 0 {
                    pc = 1;
                } else {
                    pc = 14;
                }
            }
            14 => {
                return;
            }
            15 => {
                let _o: u8 = cpu.mem[sym::VA];
                cpu.flags.z = (cpu.reg.a & _o) == 0;
                cpu.flags.n = (_o >> 7) != 0;
                if cpu.flags.n {
                    pc = 27;
                } else {
                    pc = 16;
                }
            }
            16 => {
                cpu.reg.x = 0x00;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                cpu.reg.a = cpu.mem[(cpu.mem[(sym::PAC + cpu.reg.x as usize) & 0xff] as usize | (cpu.mem[(sym::PAC + cpu.reg.x as usize + 1) & 0xff] as usize) << 8)];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::VB] = cpu.reg.a;
                let _o: u8 = cpu.mem[sym::V8];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != cpu.mem[sym::V8] {
                    pc = 24;
                } else {
                    pc = 17;
                }
            }
            17 => {
                let _v = cpu.mem[sym::PAC].wrapping_add(1);
                cpu.mem[sym::PAC] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if !cpu.flags.z {
                    pc = 19;
                } else {
                    pc = 18;
                }
            }
            18 => {
                let _v = cpu.mem[sym::PAC + 1].wrapping_add(1);
                cpu.mem[sym::PAC + 1] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 19;
            }
            19 => {
                cpu.reg.a = cpu.mem[(cpu.mem[(sym::PAC + cpu.reg.x as usize) & 0xff] as usize | (cpu.mem[(sym::PAC + cpu.reg.x as usize + 1) & 0xff] as usize) << 8)];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::V9] = cpu.reg.a;
                let _v = cpu.mem[sym::PAC].wrapping_add(1);
                cpu.mem[sym::PAC] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if !cpu.flags.z {
                    pc = 21;
                } else {
                    pc = 20;
                }
            }
            20 => {
                let _v = cpu.mem[sym::PAC + 1].wrapping_add(1);
                cpu.mem[sym::PAC + 1] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 21;
            }
            21 => {
                cpu.reg.a = cpu.mem[(cpu.mem[(sym::PAC + cpu.reg.x as usize) & 0xff] as usize | (cpu.mem[(sym::PAC + cpu.reg.x as usize + 1) & 0xff] as usize) << 8)];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::VB] = cpu.reg.a;
                let _v = cpu.mem[sym::PAC].wrapping_add(1);
                cpu.mem[sym::PAC] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if !cpu.flags.z {
                    pc = 23;
                } else {
                    pc = 22;
                }
            }
            22 => {
                let _v = cpu.mem[sym::PAC + 1].wrapping_add(1);
                cpu.mem[sym::PAC + 1] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 23;
            }
            23 => {
                cpu.reg.a = 0x80;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::VA] = cpu.reg.a;
                cpu.flags.c = false;
                if !cpu.flags.c {
                    pc = 27;
                } else {
                    pc = 24;
                }
            }
            24 => {
                cpu.reg.a = cpu.mem[sym::VB];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.a |= 0x80;
                cpu.mem[((cpu.mem[sym::PIC] as usize | (cpu.mem[sym::PIC + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.mem[sym::PAC].wrapping_add(1);
                cpu.mem[sym::PAC] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if !cpu.flags.z {
                    pc = 26;
                } else {
                    pc = 25;
                }
            }
            25 => {
                let _v = cpu.mem[sym::PAC + 1].wrapping_add(1);
                cpu.mem[sym::PAC + 1] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 26;
            }
            26 => {
                cpu.flags.c = false;
                if !cpu.flags.c {
                    pc = 9;
                } else {
                    pc = 27;
                }
            }
            27 => {
                cpu.reg.a = cpu.mem[sym::VB];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.a |= 0x80;
                cpu.mem[((cpu.mem[sym::PIC] as usize | (cpu.mem[sym::PIC + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.mem[sym::V9].wrapping_sub(1);
                cpu.mem[sym::V9] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if !cpu.flags.z {
                    pc = 9;
                } else {
                    pc = 28;
                }
            }
            28 => {
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::VA] = cpu.reg.a;
                if cpu.flags.z {
                    pc = 9;
                } else {
                    pc = 29;
                }
            }
            29 => {
                cpu.mem[sym::CrnDatPtr + 1] = cpu.reg.a;
                cpu.reg.a = 0x01;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::CrnDatPtr] = cpu.reg.a;
                WipeRgtExp(cpu);
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn DBLEXPAND(cpu: &mut Cpu) {
    cpu.mem[sym::CrnDatPtr + 1] = cpu.reg.a;
    cpu.mem[sym::CrnDatPtr] = 0x01;
    WipeRgtExp(cpu);
    return;
}

pub fn WipeRgtExp(cpu: &mut Cpu) {
    cpu.reg.a = 0x00;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::XClmPos] = cpu.reg.a;
    loop {
        cpu.mem[sym::YScrPos] = 0x00;
        ExpandClm(cpu);
        cpu.mem[sym::YScrPos] = 0x01;
        ExpandClm(cpu);
        let _v = cpu.mem[sym::XClmPos].wrapping_add(1);
        cpu.mem[sym::XClmPos] = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
        cpu.reg.a = cpu.mem[sym::XClmPos];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        let _o: u8 = 0x50;
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if !(cpu.reg.a != 0x50) {
            break;
        }
    }
    return;
}

pub fn DeltaExp(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.mem[sym::RAMRDaux] = cpu.reg.a;
                cpu.mem[sym::CrnDatPtr + 1] = cpu.reg.a;
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::CrnDatPtr] = cpu.reg.a;
                cpu.mem[sym::XClmPos] = cpu.reg.a;
                pc = 1;
            }
            1 => {
                cpu.reg.y = 0x00;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.reg.a = cpu.mem[((cpu.mem[sym::CrnDatPtr] as usize | (cpu.mem[sym::CrnDatPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0xff;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a == 0xff {
                    pc = 14;
                } else {
                    pc = 2;
                }
            }
            2 => {
                cpu.mem[sym::ByteHld] = cpu.reg.a;
                cpu.reg.a &= 0x80;
                if cpu.reg.a == 0x00 {
                    pc = 10;
                } else {
                    pc = 3;
                }
            }
            3 => {
                cpu.reg.a = cpu.mem[sym::ByteHld];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.a &= 0x7f;
                if cpu.reg.a == 0x00 {
                    pc = 7;
                } else {
                    pc = 4;
                }
            }
            4 => {
                cpu.reg.x = cpu.reg.a;
                cpu.reg.y = 0x01;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.reg.a = cpu.mem[((cpu.mem[sym::CrnDatPtr] as usize | (cpu.mem[sym::CrnDatPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                ExpClmSeq1(cpu);
                cpu.flags.c = false;
                cpu.reg.a = cpu.mem[sym::CrnDatPtr];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _r = (cpu.reg.a as u16) + (0x02) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::CrnDatPtr] = cpu.reg.a;
                if !cpu.flags.c {
                    pc = 6;
                } else {
                    pc = 5;
                }
            }
            5 => {
                let _v = cpu.mem[sym::CrnDatPtr + 1].wrapping_add(1);
                cpu.mem[sym::CrnDatPtr + 1] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 6;
            }
            6 => {
                pc = 13;
            }
            7 => {
                cpu.reg.y = 0x01;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.reg.a = cpu.mem[((cpu.mem[sym::CrnDatPtr] as usize | (cpu.mem[sym::CrnDatPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::XClmPos] = cpu.reg.a;
                cpu.reg.y = 0x02;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.reg.a = cpu.mem[((cpu.mem[sym::CrnDatPtr] as usize | (cpu.mem[sym::CrnDatPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::YScrPos] = cpu.reg.a;
                cpu.flags.c = false;
                cpu.reg.a = cpu.mem[sym::CrnDatPtr];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _r = (cpu.reg.a as u16) + (0x03) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::CrnDatPtr] = cpu.reg.a;
                if !cpu.flags.c {
                    pc = 9;
                } else {
                    pc = 8;
                }
            }
            8 => {
                let _v = cpu.mem[sym::CrnDatPtr + 1].wrapping_add(1);
                cpu.mem[sym::CrnDatPtr + 1] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 9;
            }
            9 => {
                pc = 13;
            }
            10 => {
                cpu.reg.a = cpu.mem[sym::ByteHld];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.x = 0x01;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                ExpClmSeq1(cpu);
                let _v = cpu.mem[sym::CrnDatPtr].wrapping_add(1);
                cpu.mem[sym::CrnDatPtr] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if !cpu.flags.z {
                    pc = 12;
                } else {
                    pc = 11;
                }
            }
            11 => {
                let _v = cpu.mem[sym::CrnDatPtr + 1].wrapping_add(1);
                cpu.mem[sym::CrnDatPtr + 1] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 12;
            }
            12 => {
                pc = 13;
            }
            13 => {
                cpu.reg.a = cpu.mem[sym::XClmPos];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x80;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != 0x80 {
                    pc = 1;
                } else {
                    pc = 14;
                }
            }
            14 => {
                cpu.mem[sym::RAMRDmain] = cpu.reg.a;
                pc = 15;
            }
            15 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

#[doc(alias = ":Loop")]
pub fn ExpandClm(cpu: &mut Cpu) {
    cpu.reg.y = 0x00;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    cpu.reg.a = cpu.mem[((cpu.mem[sym::CrnDatPtr] as usize | (cpu.mem[sym::CrnDatPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::ByteHld] = cpu.reg.a;
    cpu.reg.a &= 0x80;
    if cpu.reg.a == 0x00 {
        cpu.reg.a = cpu.mem[sym::ByteHld];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.reg.x = 0x01;
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
        ExpClmSeq(cpu);
        let _v = cpu.mem[sym::CrnDatPtr].wrapping_add(1);
        cpu.mem[sym::CrnDatPtr] = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
        if cpu.flags.z {
            let _v = cpu.mem[sym::CrnDatPtr + 1].wrapping_add(1);
            cpu.mem[sym::CrnDatPtr + 1] = _v;
            cpu.flags.z = _v == 0;
            cpu.flags.n = (_v >> 7) != 0;
        }
    } else {
        cpu.reg.y = 0x01;
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
        cpu.reg.a = cpu.mem[((cpu.mem[sym::CrnDatPtr] as usize | (cpu.mem[sym::CrnDatPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.reg.x = cpu.reg.a;
        cpu.reg.a = cpu.mem[sym::ByteHld];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.reg.a &= 0x7f;
        ExpClmSeq(cpu);
        cpu.flags.c = false;
        cpu.reg.a = cpu.mem[sym::CrnDatPtr];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        let _r = (cpu.reg.a as u16) + (0x02) as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::CrnDatPtr] = cpu.reg.a;
        if cpu.flags.c {
            let _v = cpu.mem[sym::CrnDatPtr + 1].wrapping_add(1);
            cpu.mem[sym::CrnDatPtr + 1] = _v;
            cpu.flags.z = _v == 0;
            cpu.flags.n = (_v >> 7) != 0;
        }
    }
    cpu.reg.a = cpu.mem[sym::YScrPos];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a < 0xc0 {
        ExpandClm(cpu);
        return;
    }
    return;
}

pub fn ExpClmSeq(cpu: &mut Cpu) {
    cpu.mem[sym::ByteHld] = cpu.reg.a;
    cpu.mem[sym::RepeatCdn] = cpu.reg.x;
    loop {
        cpu.reg.x = cpu.mem[sym::XClmPos];
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
        cpu.reg.y = cpu.mem[sym::YScrPos];
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
        cpu.reg.a = cpu.mem[sym::ByteHld];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        PutScrByte(cpu);
        cpu.reg.a = cpu.mem[sym::YScrPos];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.flags.c = false;
        let _r = (cpu.reg.a as u16) + (0x02) as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::YScrPos] = cpu.reg.a;
        let _v = cpu.mem[sym::RepeatCdn].wrapping_sub(1);
        cpu.mem[sym::RepeatCdn] = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
        if cpu.flags.z {
            break;
        }
    }
    return;
}

pub fn ExpClmSeq1(cpu: &mut Cpu) {
    cpu.mem[sym::ByteHld] = cpu.reg.a;
    cpu.mem[sym::RepeatCdn] = cpu.reg.x;
    loop {
        cpu.reg.x = cpu.mem[sym::XClmPos];
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
        cpu.reg.y = cpu.mem[sym::YScrPos];
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
        cpu.reg.a = cpu.mem[sym::ByteHld];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if (cpu.reg.a as i8) < 0 {
        } else {
            PutScrByte(cpu);
        }
        let _v = cpu.mem[sym::YScrPos].wrapping_add(1);
        cpu.mem[sym::YScrPos] = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
        cpu.reg.a = cpu.mem[sym::YScrPos];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        let _o: u8 = 0xc0;
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.a != 0xc0 {
        } else {
            cpu.reg.a = 0x00;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[sym::YScrPos] = cpu.reg.a;
            let _v = cpu.mem[sym::XClmPos].wrapping_add(1);
            cpu.mem[sym::XClmPos] = _v;
            cpu.flags.z = _v == 0;
            cpu.flags.n = (_v >> 7) != 0;
        }
        let _v = cpu.mem[sym::RepeatCdn].wrapping_sub(1);
        cpu.mem[sym::RepeatCdn] = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
        if cpu.flags.z {
            break;
        }
    }
    return;
}

pub fn PutScrByte(cpu: &mut Cpu) {
    cpu.mem[sym::ByteHld] = cpu.reg.a;
    cpu.mem[sym::ScrBasPtr] = cpu.mem[(sym::YLO + cpu.reg.y as usize) & 0xffff];
    cpu.reg.a = cpu.mem[(sym::YHI + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= 0x20;
    cpu.mem[sym::ScrBasPtr + 1] = cpu.reg.a;
    cpu.reg.a = cpu.reg.x;
    cpu.flags.c = (cpu.reg.a & 1) != 0;
    cpu.reg.a = cpu.reg.a.wrapping_shr(1);
    cpu.reg.y = cpu.reg.a;
    if !cpu.flags.c {
        cpu.mem[sym::RAMWRTaux] = cpu.reg.a;
    }
    cpu.reg.a = cpu.mem[sym::ByteHld];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[((cpu.mem[sym::ScrBasPtr] as usize | (cpu.mem[sym::ScrBasPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
    cpu.mem[sym::RAMWRTmain] = cpu.reg.a;
    return;
}

pub fn NoAuxSet(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::ByteHld];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[((cpu.mem[sym::ScrBasPtr] as usize | (cpu.mem[sym::ScrBasPtr + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
    cpu.mem[sym::RAMWRTmain] = cpu.reg.a;
    return;
}

pub fn PURPLE(cpu: &mut Cpu) {
    return;
}

pub fn DELTAEXPPOP(cpu: &mut Cpu) {
    cpu.mem[sym::PAGE2on] = cpu.reg.a;
    DeltaExp(cpu);
    cpu.mem[sym::PAGE2off] = cpu.reg.a;
    cpu.mem[sym::RAMRDaux] = cpu.reg.a;
    cpu.mem[sym::RAMWRTaux] = cpu.reg.a;
    return;
}

pub fn DELTAEXPWIPE(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.mem[sym::PAGE2off] = cpu.reg.a;
                crate::ext::_5dDE(cpu);
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn INVERTY(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.x = 0xbf;  // low line
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                cpu.reg.y = 0x00;  // high line
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                pc = 6;
            }
            1 => {
                cpu.reg.a = cpu.mem[(sym::YLO + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[(sym::YLO + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::YLO + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::YLO + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::YHI + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[(sym::YHI + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::YHI + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::YHI + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.reg.x.wrapping_sub(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _v = cpu.reg.y.wrapping_add(1);
                cpu.reg.y = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _o: u8 = 0x60;
                cpu.flags.c = cpu.reg.y >= _o;
                cpu.flags.z = cpu.reg.y == _o;
                cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.y < 0x60 {
                    pc = 6;
                } else {
                    pc = 2;
                }
            }
            2 => {
                return;
            }
            3 => {
                cpu.reg.a = 0x41;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.x = cpu.mem[sym::BBundID];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                let _o: u8 = 0xa9;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x == 0xa9 {
                    pc = 5;
                } else {
                    pc = 4;
                }
            }
            4 => {
                cpu.reg.a = 0x42;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                pc = 5;
            }
            5 => {
                cpu.mem[sym::msg2] = cpu.reg.a;
                crate::ext::blackout(cpu);
                cpu.mem[sym::RAMWRTmain] = cpu.reg.a;
                cpu.reg.x = 0x00;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 6;
            }
            6 => {
                cpu.reg.a = cpu.mem[(sym::msg1 + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x40;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a == 0x40 {
                    pc = 8;
                } else {
                    pc = 7;
                }
            }
            7 => {
                cpu.mem[(0x052a + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.reg.x.wrapping_add(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if (cpu.reg.x as i8) >= 0 {
                    pc = 6;
                } else {
                    pc = 8;
                }
            }
            8 => {
                cpu.mem[sym::RAMWRTaux] = cpu.reg.a;
                crate::ext::whoop(cpu);
                pc = 9;
            }
            9 => {
                cpu.reg.a = cpu.mem[0xc000];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.a |= cpu.mem[0xc061];
                cpu.reg.a |= cpu.mem[0xc062];
                if (cpu.reg.a as i8) >= 0 {
                    pc = 9;
                } else {
                    pc = 10;
                }
            }
            10 => {
                cpu.mem[0xc010] = cpu.reg.a;
                crate::ext::clr(cpu);
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn PROMPT(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.a = 0x41;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.x = cpu.mem[sym::BBundID];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                let _o: u8 = 0xa9;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x == 0xa9 {
                    pc = 2;
                } else {
                    pc = 1;
                }
            }
            1 => {
                cpu.reg.a = 0x42;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                pc = 2;
            }
            2 => {
                cpu.mem[sym::msg2] = cpu.reg.a;
                crate::ext::blackout(cpu);
                cpu.mem[sym::RAMWRTmain] = cpu.reg.a;
                cpu.reg.x = 0x00;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 3;
            }
            3 => {
                cpu.reg.a = cpu.mem[(sym::msg1 + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x40;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a == 0x40 {
                    pc = 5;
                } else {
                    pc = 4;
                }
            }
            4 => {
                cpu.mem[(0x052a + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.reg.x.wrapping_add(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if (cpu.reg.x as i8) >= 0 {
                    pc = 3;
                } else {
                    pc = 5;
                }
            }
            5 => {
                cpu.mem[sym::RAMWRTaux] = cpu.reg.a;
                crate::ext::whoop(cpu);
                pc = 6;
            }
            6 => {
                cpu.reg.a = cpu.mem[0xc000];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.a |= cpu.mem[0xc061];
                cpu.reg.a |= cpu.mem[0xc062];
                if (cpu.reg.a as i8) >= 0 {
                    pc = 6;
                } else {
                    pc = 7;
                }
            }
            7 => {
                cpu.mem[0xc010] = cpu.reg.a;
                crate::ext::clr(cpu);
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn CLR(cpu: &mut Cpu) {
    let _o: u8 = cpu.mem[sym::RWBANK2];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    let _o: u8 = cpu.mem[sym::RWBANK2];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    cpu.mem[0xc010] = cpu.reg.a;
    cpu.reg.a = 0x20;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::_lrcls(cpu);
    return;
}

pub fn BLACKOUT(cpu: &mut Cpu) {
    CLR(cpu);
    cpu.mem[sym::RAMRDaux] = cpu.reg.a;
    crate::ext::vblank(cpu);
    cpu.mem[sym::TEXTon] = cpu.reg.a;
    cpu.mem[sym::ADCOLoff] = cpu.reg.a;
    cpu.mem[sym::PAGE2off] = cpu.reg.a;
    return;
}

pub fn TEXT(cpu: &mut Cpu) {
    cpu.mem[sym::RAMRDaux] = cpu.reg.a;
    crate::ext::vblank(cpu);
    cpu.mem[sym::TEXTon] = cpu.reg.a;
    cpu.mem[sym::ADCOLoff] = cpu.reg.a;
    cpu.mem[sym::PAGE2off] = cpu.reg.a;
    return;
}

pub fn SETDHIRES(cpu: &mut Cpu) {
    cpu.mem[sym::RAMRDaux] = cpu.reg.a;
    cpu.mem[sym::RAMWRTaux] = cpu.reg.a;
    crate::ext::vblank(cpu);
    cpu.mem[sym::ADCOLon] = cpu.reg.a;
    let _o: u8 = cpu.mem[sym::HIRESon];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    let _o: u8 = cpu.mem[sym::DHIRESon];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    let _o: u8 = cpu.mem[sym::DHIRESoff];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    let _o: u8 = cpu.mem[sym::DHIRESon];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    let _o: u8 = cpu.mem[sym::DHIRESoff];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    let _o: u8 = cpu.mem[sym::DHIRESon];
    cpu.flags.z = (cpu.reg.a & _o) == 0;
    cpu.flags.n = (_o >> 7) != 0;
    cpu.mem[sym::TEXToff] = cpu.reg.a;
    return;
}

pub fn FADEIN(cpu: &mut Cpu) {
    cpu.mem[sym::RAMRDmain] = cpu.reg.a;
    cpu.local.insert((":sm1", 2), cpu.reg.a);
    cpu.local.insert((":sm2", 2), cpu.reg.a);
    cpu.flags.c = false;
    // 65816 (IIgs-only, not modeled): xce  ; UNPACK.S:687
    // 65816 (IIgs-only, not modeled): sep $30  ; UNPACK.S:689
    cpu.mem[0xc035] = 0x1e;
    cpu.reg.a = 0x41;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[0xc029] = cpu.reg.a;
    // 65816 (IIgs-only, not modeled): rep $30  ; UNPACK.S:696
    cpu.reg.a = 0x00;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = 0x1e;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    loop {
        let _v = cpu.reg.x.wrapping_sub(1);
        cpu.reg.x = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
        let _v = cpu.reg.x.wrapping_sub(1);
        cpu.reg.x = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
        // raw: ??? stlx $E1            ; UNPACK.S:705
        if cpu.flags.z {
            break;
        }
    }
    cpu.reg.x = 0x00;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.y = 0x00;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    cpu.reg.a = 0xff;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    // 65816 (IIgs-only, not modeled): phb  ; UNPACK.S:713
    // 65816 (IIgs-only, not modeled): mvn $E1,1  ; UNPACK.S:714
    // 65816 (IIgs-only, not modeled): plb  ; UNPACK.S:715
    // 65816 (IIgs-only, not modeled): sep $20  ; UNPACK.S:719
    cpu.reg.a = 0xc1;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[0xc029] = cpu.reg.a;
    // 65816 (IIgs-only, not modeled): rep $20  ; UNPACK.S:722
    cpu.reg.x = 0x00;  // aux mem
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.y = 0xca;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    cpu.reg.a = 0x1f;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    // 65816 (IIgs-only, not modeled): phb  ; UNPACK.S:729
    // 65816 (IIgs-only, not modeled): mvn 0,1  ; UNPACK.S:730
    // 65816 (IIgs-only, not modeled): plb  ; UNPACK.S:731
    crate::ext::PalFade(cpu);
    return;
}

pub fn LOADSUPER(cpu: &mut Cpu) {
    crate::ext::rw18(cpu);
    cpu.mem[sym::track] = 0x00;
    cpu.mem[sym::RAMWRTmain] = 0x00;
    loadscrn(cpu);
    cpu.mem[sym::RAMWRTaux] = cpu.reg.a;
    loadscrn(cpu);
    return;
}

pub fn loadscrn(cpu: &mut Cpu) {
    cpu.reg.a = 0x20;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    loop {
        cpu.local.insert((":sm", 0), cpu.reg.a);
        crate::ext::rw18(cpu);
        cpu.reg.a = cpu.local.get(&(":sm", 0)).copied().unwrap_or(0);
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.flags.c = false;
        let _r = (cpu.reg.a as u16) + (0x12) as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        let _o: u8 = 0x9e;
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if !(cpu.reg.a < 0x9e) {
            break;
        }
    }
    return;
}
