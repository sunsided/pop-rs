// @generated by pop_lifter — DO NOT EDIT.
// POP source segment `FRAMEADV` (67 routines),
// lifted to free functions over the shared `Cpu`.

use crate::cpu::Cpu;
use crate::sym;

pub fn SURE(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.a = 0x01;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::genCLS] = cpu.reg.a;
                setback(cpu);
                getprev(cpu);
                cpu.reg.a = cpu.mem[sym::SCRNUM];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                crate::ext::calcblue(cpu);
                cpu.reg.y = 0x02;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                pc = 1;
            }
            1 => {
                cpu.mem[sym::rowno] = cpu.reg.y;
                cpu.reg.a = cpu.mem[(sym::BlockBot + 1 + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Dy] = cpu.reg.a;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x03_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Ay] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::Mult10 + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::yindex] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::PREV + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::sprev + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::spreced] = cpu.reg.a;
                getbelow(cpu);
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::colno] = cpu.reg.a;
                pc = 2;
            }
            2 => {
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::XCO] = cpu.reg.a;
                cpu.mem[sym::blockxco] = cpu.reg.a;
                cpu.reg.y = cpu.mem[sym::yindex];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                getobjid(cpu);
                cpu.mem[sym::objid] = cpu.reg.a;
                RedBlockSure(cpu);
                cpu.reg.a = cpu.mem[sym::objid];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::spreced] = cpu.reg.a;
                let _v = cpu.mem[sym::yindex].wrapping_add(1);
                cpu.mem[sym::yindex] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _v = cpu.mem[sym::colno].wrapping_add(1);
                cpu.mem[sym::colno] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                cpu.reg.a = cpu.mem[sym::colno];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x0a;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a < 0x0a {
                    pc = 2;
                } else {
                    pc = 3;
                }
            }
            3 => {
                cpu.reg.y = cpu.mem[sym::rowno];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                if cpu.reg.y == 0x00 {
                    pc = 5;
                } else {
                    pc = 4;
                }
            }
            4 => {
                let _v = cpu.reg.y.wrapping_sub(1);
                cpu.reg.y = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 1;
            }
            5 => {
                cpu.reg.y = 0x02;  // bottom row of scrn above
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.mem[sym::rowno] = cpu.reg.y;
                cpu.reg.a = 0x02;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Dy] = cpu.reg.a;
                cpu.reg.a = 0xff;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Ay] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::Mult10 + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::yindex] = cpu.reg.a;
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::scrnBelow];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[sym::scrnBelowL];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[sym::SCRNUM];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelow] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::scrnLeft];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelowL] = cpu.reg.a;
                getbelow(cpu);
                cpu.reg.a = cpu.mem[sym::scrnAbove];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                crate::ext::calcblue(cpu);
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::colno] = cpu.reg.a;
                pc = 6;
            }
            6 => {
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::XCO] = cpu.reg.a;
                cpu.mem[sym::blockxco] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::scrnAbove];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    pc = 8;
                } else {
                    pc = 7;
                }
            }
            7 => {
                cpu.reg.a = 0x01;  // If screen above is null screen,
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    pc = 9;
                } else {
                    pc = 8;
                }
            }
            8 => {
                cpu.reg.y = cpu.mem[sym::yindex];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                getobjid1(cpu);
                pc = 9;
            }
            9 => {
                cpu.mem[sym::objid] = cpu.reg.a;
                RedDSure(cpu);
                cpu.reg.a = cpu.mem[sym::objid];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::spreced] = cpu.reg.a;
                let _v = cpu.mem[sym::yindex].wrapping_add(1);
                cpu.mem[sym::yindex] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _v = cpu.mem[sym::colno].wrapping_add(1);
                cpu.mem[sym::colno] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                cpu.reg.a = cpu.mem[sym::colno];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x0a;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a < 0x0a {
                    pc = 6;
                } else {
                    pc = 10;
                }
            }
            10 => {
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelowL] = cpu.reg.a;
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelow] = cpu.reg.a;
                pc = 11;
            }
            11 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn FAST(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                getprev(cpu);
                cpu.reg.a = cpu.mem[sym::SCRNUM];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                crate::ext::calcblue(cpu);
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.y = 0x14;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                metbufs3(cpu);
                cpu.mem[sym::redkidmeter] = cpu.reg.a;
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.y = 0x1c;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                metbufs2(cpu);
                cpu.mem[sym::redoppmeter] = cpu.reg.a;
                cpu.reg.a = 0x1e;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::yindex] = cpu.reg.a;
                drawobjs(cpu);
                cpu.reg.y = 0x02;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                pc = 1;
            }
            1 => {
                cpu.mem[sym::rowno] = cpu.reg.y;
                cpu.reg.a = cpu.mem[(sym::BlockBot + 1 + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Dy] = cpu.reg.a;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x03_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Ay] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::Mult10 + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::yindex] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::PREV + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::sprev + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::spreced] = cpu.reg.a;
                getbelow(cpu);
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::colno] = cpu.reg.a;
                pc = 2;
            }
            2 => {
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::XCO] = cpu.reg.a;
                cpu.mem[sym::blockxco] = cpu.reg.a;
                cpu.reg.y = cpu.mem[sym::yindex];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                getobjid(cpu);
                cpu.mem[sym::objid] = cpu.reg.a;
                RedBlockFast(cpu);
                cpu.reg.a = cpu.mem[sym::objid];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::spreced] = cpu.reg.a;
                let _v = cpu.mem[sym::yindex].wrapping_add(1);
                cpu.mem[sym::yindex] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _v = cpu.mem[sym::colno].wrapping_add(1);
                cpu.mem[sym::colno] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                cpu.reg.a = cpu.mem[sym::colno];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x0a;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a >= 0x0a {
                    pc = 4;
                } else {
                    pc = 3;
                }
            }
            3 => {
                pc = 2;
            }
            4 => {
                cpu.reg.y = cpu.mem[sym::rowno];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                if cpu.reg.y == 0x00 {
                    pc = 6;
                } else {
                    pc = 5;
                }
            }
            5 => {
                let _v = cpu.reg.y.wrapping_sub(1);
                cpu.reg.y = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                pc = 1;
            }
            6 => {
                setback(cpu);
                cpu.reg.y = 0x02;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.mem[sym::rowno] = cpu.reg.y;
                cpu.reg.a = 0x02;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Dy] = cpu.reg.a;
                cpu.reg.a = 0xff;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::Ay] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::Mult10 + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::yindex] = cpu.reg.a;
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::scrnBelow];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[sym::scrnBelowL];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[sym::SCRNUM];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelow] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::scrnLeft];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelowL] = cpu.reg.a;
                getbelow(cpu);
                cpu.reg.a = cpu.mem[sym::scrnAbove];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a == 0x00 {
                    pc = 9;
                } else {
                    pc = 7;
                }
            }
            7 => {
                crate::ext::calcblue(cpu);
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::colno] = cpu.reg.a;
                pc = 8;
            }
            8 => {
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = (cpu.reg.a >> 7) != 0;
                cpu.reg.a = cpu.reg.a.wrapping_shl(1);
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::blockxco] = cpu.reg.a;
                cpu.mem[sym::XCO] = cpu.reg.a;
                cpu.reg.y = cpu.mem[sym::yindex];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                getobjid1(cpu);
                cpu.mem[sym::objid] = cpu.reg.a;
                RedDFast(cpu);
                cpu.reg.a = cpu.mem[sym::objid];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::PRECED] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::spreced] = cpu.reg.a;
                let _v = cpu.mem[sym::yindex].wrapping_add(1);
                cpu.mem[sym::yindex] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _v = cpu.mem[sym::colno].wrapping_add(1);
                cpu.mem[sym::colno] = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                cpu.reg.a = cpu.mem[sym::colno];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x0a;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a < 0x0a {
                    pc = 8;
                } else {
                    pc = 9;
                }
            }
            9 => {
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelowL] = cpu.reg.a;
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::scrnBelow] = cpu.reg.a;
                cpu.reg.a = 0xff;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::yindex] = cpu.reg.a;
                drawobjs(cpu);
                cpu.reg.a = cpu.mem[sym::inbuilder];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    pc = 11;
                } else {
                    pc = 10;
                }
            }
            10 => {
                crate::ext::updatemeters(cpu);
                return;
            }
            11 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn RedBlockSure(cpu: &mut Cpu) {
    drawc(cpu);
    drawmc(cpu);
    drawb(cpu);
    drawmb(cpu);
    drawd(cpu);
    drawmd(cpu);
    drawa(cpu);
    drawma(cpu);
    drawfrnt(cpu);
    return;
}

pub fn RedDSure(cpu: &mut Cpu) {
    drawc(cpu);
    drawmc(cpu);
    drawb(cpu);
    drawd(cpu);
    drawmd(cpu);
    drawfrnt(cpu);
    return;
}

pub fn RedBlockFast(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[(sym::wipebuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        cpu.flags.c = true;
        let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[(sym::wipebuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
        wipesq(cpu);
        cpu.reg.y = cpu.mem[sym::yindex];
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
    }
    'b6: {
        cpu.reg.a = cpu.mem[(sym::redbuf + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a != 0x00 {
            cpu.flags.c = true;
            let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
            cpu.reg.a = _r as u8;
            cpu.flags.c = (_r >> 8) != 0;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[(sym::redbuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
            setback(cpu);
            RedBlockSure(cpu);
            cpu.reg.y = cpu.mem[sym::yindex];
            cpu.flags.z = cpu.reg.y == 0;
            cpu.flags.n = (cpu.reg.y >> 7) != 0;
            if (cpu.reg.y as i8) >= 0 {
                break 'b6;
            }
        }
        cpu.reg.a = cpu.mem[(sym::movebuf + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a != 0x00 {
            cpu.flags.c = true;
            let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
            cpu.reg.a = _r as u8;
            cpu.flags.c = (_r >> 8) != 0;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[(sym::movebuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
            setback(cpu);
            drawmc(cpu);
            drawmb(cpu);
            drawma(cpu);
            cpu.reg.y = cpu.mem[sym::yindex];
            cpu.flags.z = cpu.reg.y == 0;
            cpu.flags.n = (cpu.reg.y >> 7) != 0;
        }
    }
    'b10: {
        cpu.reg.a = cpu.mem[(sym::floorbuf + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a != 0x00 {
            cpu.flags.c = true;
            let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
            cpu.reg.a = _r as u8;
            cpu.flags.c = (_r >> 8) != 0;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[(sym::floorbuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
            setmid(cpu);
            drawfloor(cpu);
            cpu.reg.y = cpu.mem[sym::yindex];
            cpu.flags.z = cpu.reg.y == 0;
            cpu.flags.n = (cpu.reg.y >> 7) != 0;
            if (cpu.reg.y as i8) >= 0 {
                break 'b10;
            }
        }
        cpu.reg.a = cpu.mem[(sym::halfbuf + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a != 0x00 {
            cpu.flags.c = true;
            let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
            cpu.reg.a = _r as u8;
            cpu.flags.c = (_r >> 8) != 0;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[(sym::halfbuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
            setmid(cpu);
            drawhalf(cpu);
            cpu.reg.y = cpu.mem[sym::yindex];
            cpu.flags.z = cpu.reg.y == 0;
            cpu.flags.n = (cpu.reg.y >> 7) != 0;
        }
    }
    cpu.reg.a = cpu.mem[(sym::objbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        cpu.reg.a = 0x00;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[(sym::objbuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
        drawobjs(cpu);
        cpu.reg.a = cpu.mem[sym::blockxco];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::XCO] = cpu.reg.a;
        cpu.reg.y = cpu.mem[sym::yindex];
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
    }
    cpu.reg.a = cpu.mem[(sym::fredbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        cpu.flags.c = true;
        let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[(sym::fredbuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
        drawfrnt(cpu);
        cpu.reg.y = cpu.mem[sym::yindex];
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
    }
    return;
}

pub fn RedDFast(cpu: &mut Cpu) {
    cpu.reg.y = cpu.mem[sym::colno];
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::topbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        cpu.flags.c = true;
        let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[(sym::topbuf + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
        wiped(cpu);
        drawc(cpu);
        drawmc(cpu);
        drawb(cpu);
        redrawd(cpu);
        drawmd(cpu);
        drawfrnt(cpu);
    }
    return;
}

pub fn drawobjs(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.a = cpu.mem[sym::objX];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a == 0x00 {
                    pc = 10;
                } else {
                    pc = 1;
                }
            }
            1 => {
                cpu.reg.y = 0x00;  // y = sort list index
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.reg.x = 0x01;  // x = object list index
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 2;
            }
            2 => {
                cpu.reg.a = cpu.mem[(sym::objINDX + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = cpu.mem[sym::yindex];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != cpu.mem[sym::yindex] {
                    pc = 4;
                } else {
                    pc = 3;
                }
            }
            3 => {
                cpu.reg.a = cpu.reg.x;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _v = cpu.reg.y.wrapping_add(1);
                cpu.reg.y = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                cpu.mem[(sym::sortX + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
                pc = 4;
            }
            4 => {
                let _v = cpu.reg.x.wrapping_add(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _o: u8 = cpu.mem[sym::objX];
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x < cpu.mem[sym::objX] {
                    pc = 2;
                } else {
                    pc = 5;
                }
            }
            5 => {
                if cpu.flags.z {
                    pc = 2;
                } else {
                    pc = 6;
                }
            }
            6 => {
                let _o: u8 = 0x00;
                cpu.flags.c = cpu.reg.y >= _o;
                cpu.flags.z = cpu.reg.y == _o;
                cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.y == 0x00 {
                    pc = 10;
                } else {
                    pc = 7;
                }
            }
            7 => {
                cpu.mem[sym::sortX] = cpu.reg.y;
                sortlist(cpu);
                cpu.reg.x = 0x01;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 8;
            }
            8 => {
                cpu.mem[sym::xsave] = cpu.reg.x;
                cpu.reg.a = cpu.mem[(sym::sortX + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.x = cpu.reg.a;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                drawobjx(cpu);
                cpu.reg.x = cpu.mem[sym::xsave];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                let _v = cpu.reg.x.wrapping_add(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _o: u8 = cpu.mem[sym::sortX];
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x < cpu.mem[sym::sortX] {
                    pc = 8;
                } else {
                    pc = 9;
                }
            }
            9 => {
                if cpu.flags.z {
                    pc = 8;
                } else {
                    pc = 10;
                }
            }
            10 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn getprev(cpu: &mut Cpu) {
    'b2: {
        cpu.reg.a = cpu.mem[sym::SCRNUM];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a == 0x00 {
            cpu.reg.a = cpu.mem[sym::scrnLeft];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a != 0x00 {
                break 'b2;
            }
        } else {
            cpu.reg.a = cpu.mem[sym::scrnLeft];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a != 0x00 {
                break 'b2;
            }
        }
        cpu.reg.a = 0x14;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::PREV] = cpu.reg.a;
        cpu.mem[sym::PREV + 1] = cpu.reg.a;
        cpu.mem[sym::PREV + 2] = cpu.reg.a;
        cpu.reg.a = 0x00;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::sprev] = cpu.reg.a;
        cpu.mem[sym::sprev + 1] = cpu.reg.a;
        cpu.mem[sym::sprev + 2] = cpu.reg.a;
        return;
    }
    crate::ext::calcblue(cpu);
    cpu.reg.y = 0x09;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    getobjid1(cpu);
    cpu.mem[sym::PREV] = cpu.reg.a;
    cpu.mem[sym::sprev] = cpu.mem[sym::state];
    cpu.reg.y = 0x13;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    getobjid1(cpu);
    cpu.mem[sym::PREV + 1] = cpu.reg.a;
    cpu.mem[sym::sprev + 1] = cpu.mem[sym::state];
    cpu.reg.y = 0x1d;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    getobjid1(cpu);
    cpu.mem[sym::PREV + 2] = cpu.reg.a;
    cpu.reg.a = cpu.mem[sym::state];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::sprev + 2] = cpu.reg.a;
    return;
}

pub fn getbelow(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.x = cpu.mem[sym::rowno];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                let _o: u8 = 0x02;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x < 0x02 {
                    pc = 7;
                } else {
                    pc = 1;
                }
            }
            1 => {
                cpu.reg.a = cpu.mem[sym::scrnBelow];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a == 0x00 {
                    pc = 10;
                } else {
                    pc = 2;
                }
            }
            2 => {
                crate::ext::calcblue(cpu);
                cpu.reg.y = 0x08;  // skip rmost
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                pc = 3;
            }
            3 => {
                getobjid(cpu);
                cpu.mem[(sym::BELOW + 1 + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::SBELOW + 1 + cpu.reg.y as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.reg.y.wrapping_sub(1);
                cpu.reg.y = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if (cpu.reg.y as i8) >= 0 {
                    pc = 3;
                } else {
                    pc = 4;
                }
            }
            4 => {
                cpu.reg.a = cpu.mem[sym::scrnBelowL];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a == 0x00 {
                    pc = 13;
                } else {
                    pc = 5;
                }
            }
            5 => {
                crate::ext::calcblue(cpu);
                cpu.reg.y = 0x09;  // u.r. block
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                getobjid(cpu);
                cpu.mem[sym::BELOW] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::SBELOW] = cpu.reg.a;
                pc = 6;
            }
            6 => {
                cpu.reg.a = cpu.mem[sym::SCRNUM];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                crate::ext::calcblue(cpu);
                return;
            }
            7 => {
                cpu.reg.a = cpu.mem[(sym::PREV + 1 + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::BELOW] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::sprev + 1 + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::SBELOW] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::yindex];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = false;
                let _r = (cpu.reg.a as u16) + (0x0a) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.y = cpu.reg.a;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.reg.x = 0x01;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 8;
            }
            8 => {
                cpu.mem[sym::xsave] = cpu.reg.x;
                getobjid(cpu);
                cpu.reg.x = cpu.mem[sym::xsave];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                cpu.mem[(sym::BELOW + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::SBELOW + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.reg.y.wrapping_add(1);
                cpu.reg.y = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _v = cpu.reg.x.wrapping_add(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _o: u8 = 0x0a;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x < 0x0a {
                    pc = 8;
                } else {
                    pc = 9;
                }
            }
            9 => {
                return;
            }
            10 => {
                cpu.reg.a = 0x01;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.x = cpu.reg.a;
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 11;
            }
            11 => {
                cpu.mem[(sym::BELOW + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                let _v = cpu.reg.x.wrapping_add(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                let _o: u8 = 0x0a;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x < 0x0a {
                    pc = 11;
                } else {
                    pc = 12;
                }
            }
            12 => {
                if cpu.flags.c {
                    pc = 4;
                } else {
                    pc = 13;
                }
            }
            13 => {
                cpu.reg.a = cpu.mem[sym::level];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = 0x0c;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a == 0x0c {
                    pc = 16;
                } else {
                    pc = 14;
                }
            }
            14 => {
                cpu.reg.a = 0x14;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                pc = 15;
            }
            15 => {
                cpu.mem[sym::BELOW] = cpu.reg.a;
                if !cpu.flags.n {
                    pc = 6;
                } else {
                    pc = 16;
                }
            }
            16 => {
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if (cpu.reg.a as i8) >= 0 {
                    pc = 15;
                } else {
                    pc = 17;
                }
            }
            17 => {
                cpu.reg.a = cpu.mem[(sym::objX + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::FCharX] = cpu.reg.a;
                cpu.mem[sym::XCO] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objOFF + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::OFFSET] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objY + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::FCharY] = cpu.reg.a;
                cpu.mem[sym::YCO] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objIMG + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objTAB + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::TABLE] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objFACE + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::FCharFace] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objCU + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::FCharCU] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objCD + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::FCharCD] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objCL + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::FCharCL] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objCR + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::FCharCR] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::objTYP + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                pc = 18;
            }
            18 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn loadobj(cpu: &mut Cpu) {
    cpu.mem[sym::FCharX] = cpu.mem[(sym::objX + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::XCO] = cpu.mem[(sym::objX + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::OFFSET] = cpu.mem[(sym::objOFF + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::FCharY] = cpu.mem[(sym::objY + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::YCO] = cpu.mem[(sym::objY + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::IMAGE] = cpu.mem[(sym::objIMG + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::TABLE] = cpu.mem[(sym::objTAB + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::FCharFace] = cpu.mem[(sym::objFACE + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::FCharCU] = cpu.mem[(sym::objCU + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::FCharCD] = cpu.mem[(sym::objCD + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::FCharCL] = cpu.mem[(sym::objCL + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::FCharCR] = cpu.mem[(sym::objCR + cpu.reg.x as usize) & 0xffff];
    cpu.reg.a = cpu.mem[(sym::objTYP + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn drawfrnt(cpu: &mut Cpu) {
    cpu.reg.x = cpu.mem[sym::PRECED];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if cpu.reg.x == 0x04 {
        DrawGateBF_3f(cpu);
    }
    cpu.reg.x = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if cpu.reg.x != 0x12 {
        'b9: {
            if cpu.reg.x == 0x0a {
                cpu.reg.a = cpu.mem[sym::state];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.a &= 0xe0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0xa0 {
                    if cpu.reg.a >= 0x40 {
                        cpu.reg.a = 0x95;
                        cpu.flags.z = cpu.reg.a == 0;
                        cpu.flags.n = (cpu.reg.a >> 7) != 0;
                        if cpu.reg.a != 0x00 {
                            break 'b9;
                        }
                    }
                }
            }
            cpu.reg.x = cpu.mem[sym::objid];
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            cpu.reg.a = cpu.mem[(sym::fronti + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a == 0x00 {
                return;
            }
        }
        'b18: {
            cpu.mem[sym::IMAGE] = cpu.reg.a;
            cpu.reg.a = cpu.mem[sym::Ay];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = false;
            let _r = (cpu.reg.a as u16) + cpu.mem[(sym::fronty + cpu.reg.x as usize) & 0xffff] as u16 + (cpu.flags.c as u16);
            cpu.reg.a = _r as u8;
            cpu.flags.c = (_r >> 8) != 0;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[sym::YCO] = cpu.reg.a;
            cpu.reg.a = cpu.mem[sym::blockxco];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = false;
            let _r = (cpu.reg.a as u16) + cpu.mem[(sym::frontx + cpu.reg.x as usize) & 0xffff] as u16 + (cpu.flags.c as u16);
            cpu.reg.a = _r as u8;
            cpu.flags.c = (_r >> 8) != 0;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[sym::XCO] = cpu.reg.a;
            if cpu.reg.x < 0x1b {
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x02 {
                    cpu.reg.a = cpu.mem[sym::BGset1];
                    cpu.flags.z = cpu.reg.a == 0;
                    cpu.flags.n = (cpu.reg.a >> 7) != 0;
                    if cpu.reg.a != 0x01 {
                        if cpu.reg.x == 0x03 {
                            break 'b18;
                        }
                    }
                }
                if cpu.reg.x == 0x14 {
                    cpu.reg.y = cpu.mem[sym::state];
                    cpu.flags.z = cpu.reg.y == 0;
                    cpu.flags.n = (cpu.reg.y >> 7) != 0;
                    if cpu.reg.y >= 0x02 {
                        cpu.reg.y = 0x00;
                        cpu.flags.z = cpu.reg.y == 0;
                        cpu.flags.n = (cpu.reg.y >> 7) != 0;
                    }
                    cpu.reg.a = cpu.mem[(sym::blockfr + cpu.reg.y as usize) & 0xffff];
                    cpu.flags.z = cpu.reg.a == 0;
                    cpu.flags.n = (cpu.reg.a >> 7) != 0;
                    cpu.mem[sym::IMAGE] = cpu.reg.a;
                } else {
                    maddfore(cpu);
                    return;
                }
            }
        }
        cpu.reg.x = 0x02;
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
        cpu.mem[sym::OPACITY] = cpu.reg.x;
        crate::ext::addfore(cpu);
        return;
    }
    drawslicerf(cpu);
    return;
}

pub fn DrawGateBF_3f(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::rowno];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != cpu.mem[sym::KidBlockY] {
        return;
    }
    cpu.reg.x = cpu.mem[sym::colno];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    let _v = cpu.reg.x.wrapping_sub(1);
    cpu.reg.x = _v;
    cpu.flags.z = _v == 0;
    cpu.flags.n = (_v >> 7) != 0;
    if cpu.reg.x != cpu.mem[sym::KidBlockX] {
        return;
    }
    cpu.reg.a = cpu.mem[sym::scrnRight];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == cpu.mem[sym::KidScrn] {
        return;
    }
    drawgatebf(cpu);
    return;
}

pub fn drawmb(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::PRECED];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x04 {
        if cpu.reg.a != 0x02 {
            if cpu.reg.a != 0x0b {
                if cpu.reg.a != 0x13 {
                    if cpu.reg.a != 0x10 {
                        return;
                    }
                    drawexitb(cpu);
                    return;
                }
                drawtorchb(cpu);
                return;
            }
            drawlooseb(cpu);
            return;
        }
        drawspikeb(cpu);
        return;
    }
    drawgateb(cpu);
    return;
}

pub fn drawmc(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    match cpu.reg.a {
        0x00 | 0x0c => {
            cpu.reg.x = cpu.mem[sym::colno];
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            cpu.reg.a = cpu.mem[(sym::BELOW + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a != 0x04 {
                return;
            }
            drawgatec(cpu);
            return;
        }
        _ => {}
    }
    let _o: u8 = 0x09;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a != 0x09 {
        if !cpu.flags.z {
            return;
        }
    }
    cpu.reg.x = cpu.mem[sym::colno];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::BELOW + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x04 {
        return;
    }
    drawgatec(cpu);
    return;
}

pub fn drawc(cpu: &mut Cpu) {
    checkc(cpu);
    if !cpu.flags.c {
        return;
    }
    dodrawc(cpu);
    domaskb(cpu);
    return;
}

pub fn checkc(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    match cpu.reg.a {
        0x00 | 0x09 | 0x0c => {
            cpu.flags.c = true;
            return;
        }
        _ => {}
    }
    let _o: u8 = 0x1a;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a < 0x1a {
        if !cpu.flags.c {
            return;
        }
    }
    cpu.flags.c = true;
    return;
}

pub fn dodrawc(cpu: &mut Cpu) {
    cpu.reg.x = cpu.mem[sym::colno];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::BELOW + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = cpu.reg.a;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    let _o: u8 = 0x14;
    cpu.flags.c = cpu.reg.x >= _o;
    cpu.flags.z = cpu.reg.x == _o;
    cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.x == 0x14 {
        cpu.reg.x = cpu.mem[sym::colno];
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
        cpu.reg.a = cpu.mem[(sym::SBELOW + cpu.reg.x as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.reg.y = cpu.reg.a;
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
        let _o: u8 = 0x02;
        cpu.flags.c = cpu.reg.y >= _o;
        cpu.flags.z = cpu.reg.y == _o;
        cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.y >= 0x02 {
            cpu.reg.y = 0x00;
            cpu.flags.z = cpu.reg.y == 0;
            cpu.flags.n = (cpu.reg.y >> 7) != 0;
        }
        cpu.reg.a = cpu.mem[(sym::blockc + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a == 0x00 {
            return;
        }
    } else {
        cpu.reg.a = cpu.mem[(sym::piecec + cpu.reg.x as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a == 0x00 {
            return;
        }
        let _o: u8 = 0x9f;
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.a == 0x9f {
            cpu.reg.x = cpu.mem[sym::colno];
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            cpu.reg.a = cpu.mem[(sym::SBELOW + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.y = cpu.reg.a;
            cpu.flags.z = cpu.reg.y == 0;
            cpu.flags.n = (cpu.reg.y >> 7) != 0;
            let _o: u8 = 0x03;
            cpu.flags.c = cpu.reg.y >= _o;
            cpu.flags.z = cpu.reg.y == _o;
            cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
            if cpu.reg.y >= 0x03 {
                return;
            }
            cpu.reg.a = cpu.mem[(sym::panelc + cpu.reg.y as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a == 0x00 {
                return;
            }
        }
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.mem[sym::YCO] = cpu.mem[sym::Dy];
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn domaskb(cpu: &mut Cpu) {
    cpu.reg.x = cpu.mem[sym::PRECED];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::maskb + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::YCO] = cpu.mem[sym::Dy];
    cpu.reg.a = 0x00;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawb(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _o: u8 = 0x14;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a == 0x14 {
        return;
    }
    'b25: {
        'b24: {
            cpu.reg.x = cpu.mem[sym::PRECED];
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            let _o: u8 = 0x00;
            cpu.flags.c = cpu.reg.x >= _o;
            cpu.flags.z = cpu.reg.x == _o;
            cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
            if cpu.reg.x == 0x00 {
                cpu.reg.y = cpu.mem[sym::spreced];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                let _o: u8 = 0x04;
                cpu.flags.c = cpu.reg.y >= _o;
                cpu.flags.z = cpu.reg.y == _o;
                cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.y >= 0x04 {
                    return;
                }
                cpu.reg.a = cpu.mem[(sym::spaceb + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a == 0x00 {
                    return;
                }
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::spaceby + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
            } else {
                'b16: {
                    let _o: u8 = 0x01;
                    cpu.flags.c = cpu.reg.x >= _o;
                    cpu.flags.z = cpu.reg.x == _o;
                    cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                    if cpu.reg.x != 0x01 {
                        let _o: u8 = 0x14;
                        cpu.flags.c = cpu.reg.x >= _o;
                        cpu.flags.z = cpu.reg.x == _o;
                        cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                        if cpu.reg.x == 0x14 {
                            cpu.reg.y = cpu.mem[sym::spreced];
                            cpu.flags.z = cpu.reg.y == 0;
                            cpu.flags.n = (cpu.reg.y >> 7) != 0;
                            let _o: u8 = 0x02;
                            cpu.flags.c = cpu.reg.y >= _o;
                            cpu.flags.z = cpu.reg.y == _o;
                            cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
                            if cpu.reg.y >= 0x02 {
                                cpu.reg.y = 0x00;
                                cpu.flags.z = cpu.reg.y == 0;
                                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                            }
                            cpu.reg.a = cpu.mem[(sym::blockb + cpu.reg.y as usize) & 0xffff];
                            cpu.flags.z = cpu.reg.a == 0;
                            cpu.flags.n = (cpu.reg.a >> 7) != 0;
                            if cpu.reg.a == 0x00 {
                                break 'b16;
                            }
                        } else {
                            cpu.reg.a = cpu.mem[(sym::pieceb + cpu.reg.x as usize) & 0xffff];
                            cpu.flags.z = cpu.reg.a == 0;
                            cpu.flags.n = (cpu.reg.a >> 7) != 0;
                            if cpu.reg.a == 0x00 {
                                crate::ext::_3astripe(cpu);
                                return;
                            }
                            let _o: u8 = 0x9e;
                            cpu.flags.c = cpu.reg.a >= _o;
                            cpu.flags.z = cpu.reg.a == _o;
                            cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                            if cpu.reg.a == 0x9e {
                                cpu.reg.y = cpu.mem[sym::spreced];
                                cpu.flags.z = cpu.reg.y == 0;
                                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                                let _o: u8 = 0x03;
                                cpu.flags.c = cpu.reg.y >= _o;
                                cpu.flags.z = cpu.reg.y == _o;
                                cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
                                if cpu.reg.y >= 0x03 {
                                    return;
                                }
                                cpu.reg.a = cpu.mem[(sym::panelb + cpu.reg.y as usize) & 0xffff];
                                cpu.flags.z = cpu.reg.a == 0;
                                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                                if cpu.reg.a == 0x00 {
                                    return;
                                }
                            } else {
                                _3acont1(cpu);
                                cpu.reg.a = 0x00;
                                cpu.flags.z = cpu.reg.a == 0;
                                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                                let _o: u8 = 0x02;
                                cpu.flags.c = cpu.reg.a >= _o;
                                cpu.flags.z = cpu.reg.a == _o;
                                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                                if cpu.reg.a == 0x02 {
                                    crate::ext::_3astripe(cpu);
                                    return;
                                }
                                cpu.reg.a = cpu.mem[sym::BGset1];
                                cpu.flags.z = cpu.reg.a == 0;
                                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                                let _o: u8 = 0x01;
                                cpu.flags.c = cpu.reg.a >= _o;
                                cpu.flags.z = cpu.reg.a == _o;
                                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                                if cpu.reg.a != 0x01 {
                                    return;
                                }
                                cpu.reg.x = cpu.mem[sym::PRECED];
                                cpu.flags.z = cpu.reg.x == 0;
                                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                                cpu.reg.a = cpu.mem[(sym::bstripe + cpu.reg.x as usize) & 0xffff];
                                cpu.flags.z = cpu.reg.a == 0;
                                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                                if cpu.reg.a == 0x00 {
                                    return;
                                }
                                cpu.mem[sym::IMAGE] = cpu.reg.a;
                                cpu.reg.a = cpu.mem[sym::Ay];
                                cpu.flags.z = cpu.reg.a == 0;
                                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                                cpu.flags.c = true;
                                let _r = (cpu.reg.a as u16) + (!0x20_u8) as u16 + (cpu.flags.c as u16);
                                cpu.reg.a = _r as u8;
                                cpu.flags.c = (_r >> 8) != 0;
                                cpu.flags.z = cpu.reg.a == 0;
                                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                                break 'b25;
                            }
                        }
                        cpu.mem[sym::IMAGE] = cpu.reg.a;
                        cpu.reg.a = cpu.mem[(sym::pieceby + cpu.reg.x as usize) & 0xffff];
                        cpu.flags.z = cpu.reg.a == 0;
                        cpu.flags.n = (cpu.reg.a >> 7) != 0;
                        break 'b24;
                    }
                }
                cpu.reg.y = cpu.mem[sym::spreced];
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                let _o: u8 = 0x04;
                cpu.flags.c = cpu.reg.y >= _o;
                cpu.flags.z = cpu.reg.y == _o;
                cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.y >= 0x04 {
                    cpu.reg.y = 0x00;
                    cpu.flags.z = cpu.reg.y == 0;
                    cpu.flags.n = (cpu.reg.y >> 7) != 0;
                }
                cpu.reg.a = cpu.mem[(sym::floorb + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a == 0x00 {
                    return;
                }
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                cpu.reg.a = cpu.mem[(sym::floorby + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
            }
        }
        cpu.flags.c = false;
        let _r = (cpu.reg.a as u16) + cpu.mem[sym::Ay] as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
    }
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn redrawd(cpu: &mut Cpu) {
    drawd(cpu);
    if cpu.flags.z {
        return;
    }
    crate::ext::addfore(cpu);
    return;
}

pub fn drawd(cpu: &mut Cpu) {
    cpu.mem[sym::OPACITY] = 0x02;
    cpu.reg.x = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    let _o: u8 = 0x14;
    cpu.flags.c = cpu.reg.x >= _o;
    cpu.flags.z = cpu.reg.x == _o;
    cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.x == 0x14 {
        cpu.reg.y = cpu.mem[sym::state];
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
        let _o: u8 = 0x02;
        cpu.flags.c = cpu.reg.y >= _o;
        cpu.flags.z = cpu.reg.y == _o;
        cpu.flags.n = (cpu.reg.y.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.y >= 0x02 {
            cpu.reg.y = 0x00;
            cpu.flags.z = cpu.reg.y == 0;
            cpu.flags.n = (cpu.reg.y >> 7) != 0;
        }
        cpu.reg.a = cpu.mem[(sym::blockd + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a == 0x00 {
            cpu.reg.a = cpu.mem[sym::PRECED];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            let _o: u8 = 0x1a;
            cpu.flags.c = cpu.reg.a >= _o;
            cpu.flags.z = cpu.reg.a == _o;
            cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
            if cpu.reg.a == 0x1a {
                cpu.reg.x = cpu.mem[sym::objid];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                let _o: u8 = 0x0c;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x != 0x0c {
                    adda(cpu);
                    return;
                }
                cpu.reg.a = 0xa1;  // arch ends to L of panel
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                adda1(cpu);
                return;
            }
            let _o: u8 = 0x07;
            cpu.flags.c = cpu.reg.a >= _o;
            cpu.flags.z = cpu.reg.a == _o;
            cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
            if cpu.reg.a != 0x07 {
                let _o: u8 = 0x0c;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != 0x0c {
                    let _o: u8 = 0x09;
                    cpu.flags.c = cpu.reg.a >= _o;
                    cpu.flags.z = cpu.reg.a == _o;
                    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                    if cpu.reg.a != 0x09 {
                        let _o: u8 = 0x14;
                        cpu.flags.c = cpu.reg.a >= _o;
                        cpu.flags.z = cpu.reg.a == _o;
                        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                        if cpu.reg.a != 0x14 {
                            adda(cpu);
                            return;
                        }
                    }
                }
            }
            addamask(cpu);
            adda(cpu);
            return;
        }
    } else {
        let _o: u8 = 0x0c;
        cpu.flags.c = cpu.reg.x >= _o;
        cpu.flags.z = cpu.reg.x == _o;
        cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.x == 0x0c {
            cpu.reg.a = 0x01;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[sym::OPACITY] = cpu.reg.a;
        }
        cpu.reg.a = cpu.mem[(sym::pieced + cpu.reg.x as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a == 0x00 {
            return;
        }
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = cpu.mem[sym::Dy];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    add(cpu);
    cpu.reg.a = 0xff;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn drawa(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::PRECED];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _o: u8 = 0x1a;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a == 0x1a {
        cpu.reg.x = cpu.mem[sym::objid];
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
        let _o: u8 = 0x0c;
        cpu.flags.c = cpu.reg.x >= _o;
        cpu.flags.z = cpu.reg.x == _o;
        cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.x != 0x0c {
            adda(cpu);
            return;
        }
        cpu.reg.a = 0xa1;  // arch ends to L of panel
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        adda1(cpu);
        return;
    }
    let _o: u8 = 0x07;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a != 0x07 {
        let _o: u8 = 0x0c;
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.a != 0x0c {
            let _o: u8 = 0x09;
            cpu.flags.c = cpu.reg.a >= _o;
            cpu.flags.z = cpu.reg.a == _o;
            cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
            if cpu.reg.a != 0x09 {
                let _o: u8 = 0x14;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a != 0x14 {
                    adda(cpu);
                    return;
                }
            }
        }
    }
    addamask(cpu);
    adda(cpu);
    return;
}

pub fn addmidezfast(cpu: &mut Cpu) {
    cpu.reg.a = 0x00;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::addmidez(cpu);
    return;
}

#[doc(alias = "]add")]
pub fn add(cpu: &mut Cpu) {
    crate::ext::addback(cpu);
    return;
}

pub fn setback(cpu: &mut Cpu) {
    cpu.reg.a = 0x99;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.local.insert(("]add", 1), cpu.reg.a);
    cpu.reg.a = 0x04;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.local.insert(("]add", 2), cpu.reg.a);
    return;
}

pub fn setmid(cpu: &mut Cpu) {
    cpu.reg.a = 0x2c;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.local.insert(("]add", 1), cpu.reg.a);
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.local.insert(("]add", 2), cpu.reg.a);
    return;
}

pub fn maddfore(cpu: &mut Cpu) {
    cpu.reg.x = 0x04;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.x;
    crate::ext::addfore(cpu);
    cpu.reg.x = 0x01;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.x;
    crate::ext::addfore(cpu);
    return;
}

pub fn addamask(cpu: &mut Cpu) {
    cpu.reg.x = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::maska + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.mem[sym::YCO] = cpu.mem[sym::Ay];
    cpu.reg.a = 0x00;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn adda(cpu: &mut Cpu) {
    cpu.reg.x = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    getpiecea(cpu);
    if cpu.flags.z {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + cpu.mem[(sym::pieceay + cpu.reg.x as usize) & 0xffff] as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn adda1(cpu: &mut Cpu) {
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + cpu.mem[(sym::pieceay + cpu.reg.x as usize) & 0xffff] as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawma(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x02 {
        if cpu.reg.a != 0x12 {
            if cpu.reg.a != 0x0a {
                if cpu.reg.a != 0x16 {
                    return;
                }
                drawsworda(cpu);
                return;
            }
            drawflaska(cpu);
            return;
        }
        drawslicera(cpu);
        return;
    }
    drawspikea(cpu);
    return;
}

pub fn drawmd(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x0b {
        return;
    }
    drawloosed(cpu);
    return;
}

pub fn drawfloor(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::PRECED];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        return;
    }
    addamask(cpu);
    adda(cpu);
    drawma(cpu);
    drawd(cpu);
    return;
}

pub fn drawhalf(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::PRECED];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        return;
    }
    'b10: {
        cpu.reg.x = cpu.mem[sym::objid];
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
        if cpu.reg.x != 0x01 {
            if cpu.reg.x != 0x13 {
                if cpu.reg.x != 0x05 {
                    if cpu.reg.x != 0x10 {
                        cpu.reg.a = cpu.mem[sym::BGset1];
                        cpu.flags.z = cpu.reg.a == 0;
                        cpu.flags.n = (cpu.reg.a >> 7) != 0;
                        if cpu.reg.a != 0x01 {
                            crate::ext::_5ddrawflr(cpu);
                            return;
                        }
                        if cpu.reg.x != 0x03 {
                            if cpu.reg.x != 0x19 {
                                crate::ext::_5ddrawflr(cpu);
                                return;
                            }
                        }
                        _3asub(cpu);
                        cpu.reg.a = 0x0e;
                        cpu.flags.z = cpu.reg.a == 0;
                        cpu.flags.n = (cpu.reg.a >> 7) != 0;
                        if cpu.reg.a != 0x00 {
                            break 'b10;
                        }
                    }
                }
            }
        }
        _3asub(cpu);
        cpu.reg.a = 0x12;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    drawd(cpu);
    return;
}

pub fn wipesq(cpu: &mut Cpu) {
    cpu.mem[sym::height] = cpu.mem[(sym::whitebuf + cpu.reg.y as usize) & 0xffff];
    cpu.mem[sym::width] = 0x04;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.mem[sym::YCO] = cpu.mem[sym::Dy];
    cpu.reg.a = 0x80;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::addwipe(cpu);
    return;
}

pub fn wiped(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.a = cpu.mem[sym::objid];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a == 0x09 {
                    pc = 5;
                } else {
                    pc = 1;
                }
            }
            1 => {
                if cpu.reg.a == 0x07 {
                    pc = 5;
                } else {
                    pc = 2;
                }
            }
            2 => {
                if cpu.reg.a == 0x0c {
                    pc = 5;
                } else {
                    pc = 3;
                }
            }
            3 => {
                let _o: u8 = 0x14;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a == 0x14 {
                    pc = 5;
                } else {
                    pc = 4;
                }
            }
            4 => {
                cpu.reg.a = 0x03;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                crate::ext::_5dwipe(cpu);
                return;
            }
            5 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn drawloosed(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::state];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    getloosey(cpu);
    cpu.reg.a = cpu.mem[(sym::loosed + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.mem[sym::YCO] = cpu.mem[sym::Dy];
    cpu.reg.a = 0x02;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawlooseb(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::spreced];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    getloosey(cpu);
    cpu.mem[sym::IMAGE] = 0x1b;
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + cpu.mem[(sym::looseby + cpu.reg.y as usize) & 0xffff] as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn getpiecea(cpu: &mut Cpu) {
    if cpu.reg.x == 0x0b {
        cpu.reg.a = cpu.mem[sym::state];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        getloosey(cpu);
        cpu.reg.a = cpu.mem[(sym::loosea + cpu.reg.y as usize) & 0xffff];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        return;
    }
    cpu.reg.a = cpu.mem[(sym::piecea + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn drawspikea(cpu: &mut Cpu) {
    cpu.reg.x = cpu.mem[sym::state];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if (cpu.reg.x as i8) < 0 {
        cpu.reg.x = 0x05;  // hibit set --> spikes extended
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
    }
    cpu.reg.a = cpu.mem[(sym::spikea + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawspikeb(cpu: &mut Cpu) {
    cpu.reg.x = cpu.mem[sym::spreced];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if (cpu.reg.x as i8) < 0 {
        cpu.reg.x = 0x05;  // hibit set --> spikes extended
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
    }
    cpu.reg.a = cpu.mem[(sym::spikeb + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!0x01_u8) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawsworda(cpu: &mut Cpu) {
    cpu.reg.a = 0x99;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = cpu.mem[sym::state];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if cpu.reg.x == 0x01 {
        cpu.reg.a = 0xb3;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.mem[sym::YCO] = cpu.mem[sym::Ay];
    cpu.reg.a = 0x02;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawslicera(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::state];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a &= 0x7f;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = cpu.reg.a;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if cpu.reg.x >= 0x06 {
        cpu.reg.x = 0x06;  // fully retracted
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
    }
    'b6: {
        'b5: {
            cpu.reg.a = cpu.mem[(sym::slicerseq + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.x = cpu.reg.a;
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            let _v = cpu.reg.x.wrapping_sub(1);
            cpu.reg.x = _v;
            cpu.flags.z = _v == 0;
            cpu.flags.n = (_v >> 7) != 0;
            cpu.mem[sym::xsave] = cpu.reg.x;
            cpu.reg.a = cpu.mem[sym::blockxco];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[sym::XCO] = cpu.reg.a;
            cpu.reg.a = cpu.mem[sym::Ay];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.mem[sym::YCO] = cpu.reg.a;
            cpu.reg.a = cpu.mem[sym::state];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if (cpu.reg.a as i8) < 0 {
                cpu.reg.a = cpu.mem[(sym::slicerbot2 + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    break 'b5;
                }
            }
            cpu.reg.a = cpu.mem[(sym::slicerbot + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a == 0x00 {
                break 'b6;
            }
        }
        cpu.mem[sym::IMAGE] = cpu.reg.a;
        cpu.reg.a = 0x01;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::OPACITY] = cpu.reg.a;
        add(cpu);
        cpu.reg.x = cpu.mem[sym::xsave];
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
    }
    cpu.reg.a = cpu.mem[(sym::slicertop + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!cpu.mem[(sym::slicergap + cpu.reg.x as usize) & 0xffff]) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawslicerf(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::state];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a &= 0x7f;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = cpu.reg.a;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if cpu.reg.x >= 0x06 {
        cpu.reg.x = 0x06;  // fully retracted
        cpu.flags.z = cpu.reg.x == 0;
        cpu.flags.n = (cpu.reg.x >> 7) != 0;
    }
    cpu.reg.a = cpu.mem[(sym::slicerseq + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = cpu.reg.a;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    let _v = cpu.reg.x.wrapping_sub(1);
    cpu.reg.x = _v;
    cpu.flags.z = _v == 0;
    cpu.flags.n = (_v >> 7) != 0;
    cpu.mem[sym::xsave] = cpu.reg.x;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.mem[sym::YCO] = cpu.mem[sym::Ay];
    cpu.reg.a = cpu.mem[(sym::slicerfrnt + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    maddfore(cpu);
    return;
}

pub fn drawexitb(cpu: &mut Cpu) {
    cpu.mem[sym::IMAGE] = 0x6b;
    cpu.mem[sym::YCO] = (cpu.mem[sym::Ay]).wrapping_sub(0x0c);
    cpu.reg.a = cpu.mem[sym::blockxco];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _o: u8 = 0x24;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a >= 0x24 {
        return;
    }
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + (0x01) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::XCO] = cpu.reg.a;
    cpu.mem[sym::OPACITY] = 0x02;
    cpu.reg.a = cpu.mem[sym::SCRNUM];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _o: u8 = cpu.mem[sym::KidStartScrn];
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a == cpu.mem[sym::KidStartScrn] {
    } else {
        add(cpu);
    }
    cpu.reg.a = cpu.mem[sym::Dy];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!0x43_u8) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _o: u8 = 0xc0;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a >= 0xc0 {
        return;
    }
    cpu.mem[sym::blockthr] = cpu.reg.a;
    cpu.mem[sym::gateposn] = (cpu.mem[sym::spreced]).wrapping_shr(0x02 as u32);
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!0x0e_u8) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _r = (cpu.reg.a as u16) + (!cpu.mem[sym::gateposn]) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::doortop] = cpu.reg.a;
    loop {
        cpu.mem[sym::YCO] = cpu.reg.a;
        cpu.mem[sym::IMAGE] = 0x6d;
        cpu.reg.a = 0x00;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::OPACITY] = cpu.reg.a;
        add(cpu);
        cpu.mem[sym::IMAGE] = 0x6c;
        cpu.reg.a = 0x01;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.mem[sym::OPACITY] = cpu.reg.a;
        add(cpu);
        cpu.reg.a = cpu.mem[sym::YCO];
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        cpu.flags.c = true;
        let _r = (cpu.reg.a as u16) + (!0x04_u8) as u16 + (cpu.flags.c as u16);
        cpu.reg.a = _r as u8;
        cpu.flags.c = (_r >> 8) != 0;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        let _o: u8 = cpu.mem[sym::blockthr];
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if !(cpu.reg.a >= cpu.mem[sym::blockthr]) {
            break;
        }
    }
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!0x40_u8) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _o: u8 = 0xc0;
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a >= 0xc0 {
        return;
    }
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.mem[sym::IMAGE] = 0x6e;
    cpu.reg.a = 0x02;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn drawgatec(cpu: &mut Cpu) {
    cpu.mem[sym::YCO] = cpu.mem[sym::Dy];
    cpu.mem[sym::IMAGE] = 0x0d;
    cpu.reg.a = 0x00;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    cpu.reg.x = cpu.mem[sym::colno];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::SBELOW + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a >= 0xbc {
        cpu.reg.a = 0xbc;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
    }
    cpu.flags.c = (cpu.reg.a & 1) != 0;
    cpu.reg.a = cpu.reg.a.wrapping_shr(1);
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = (cpu.reg.a & 1) != 0;
    cpu.reg.a = cpu.reg.a.wrapping_shr(1);
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::gateposn] = cpu.reg.a;
    cpu.reg.a &= 0xf8;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a ^= 0xff;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + (0x01) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + cpu.mem[sym::gateposn] as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.y = cpu.reg.a;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    cpu.mem[sym::IMAGE] = cpu.mem[(sym::gate8c + cpu.reg.y as usize) & 0xffff];
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn setupdgb(cpu: &mut Cpu) {
    cpu.mem[sym::blockthr] = (cpu.mem[sym::Dy]).wrapping_sub(0x3e);
    cpu.reg.a = cpu.mem[sym::spreced];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a >= 0xbc {
        cpu.reg.a = 0xbc;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
    }
    cpu.flags.c = (cpu.reg.a & 1) != 0;
    cpu.reg.a = cpu.reg.a.wrapping_shr(1);
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = (cpu.reg.a & 1) != 0;
    cpu.reg.a = cpu.reg.a.wrapping_shr(1);
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + (0x01) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::gateposn] = cpu.reg.a;
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!cpu.mem[sym::gateposn]) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::gatebot] = cpu.reg.a;
    return;
}

pub fn drawgatebf(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                setupdgb(cpu);
                cpu.reg.a = 0x01;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::OPACITY] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::gatebot];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x02_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::YCO] = cpu.reg.a;
                cpu.reg.a = 0x44;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                crate::ext::addfore(cpu);
                pc = 1;
            }
            1 => {
                cpu.reg.a = 0x37;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::gatebot];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x0c_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                pc = 2;
            }
            2 => {
                cpu.mem[sym::YCO] = cpu.reg.a;
                let _o: u8 = 0xc0;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a >= 0xc0 {
                    pc = 7;
                } else {
                    pc = 3;
                }
            }
            3 => {
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x07_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if !cpu.flags.c {
                    pc = 6;
                } else {
                    pc = 4;
                }
            }
            4 => {
                let _o: u8 = cpu.mem[sym::blockthr];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a < cpu.mem[sym::blockthr] {
                    pc = 6;
                } else {
                    pc = 5;
                }
            }
            5 => {
                crate::ext::addfore(cpu);
                cpu.reg.a = cpu.mem[sym::YCO];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x08_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    pc = 2;
                } else {
                    pc = 6;
                }
            }
            6 => {
                pc = 7;
            }
            7 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn drawgateb(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                setupdgb(cpu);
                cpu.flags.c = false;
                let _r = (cpu.reg.a as u16) + (0x0c) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                let _o: u8 = cpu.mem[sym::Ay];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a < cpu.mem[sym::Ay] {
                    pc = 2;
                } else {
                    pc = 1;
                }
            }
            1 => {
                restorebot(cpu);
                cpu.reg.a = cpu.mem[sym::gatebot];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x02_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::YCO] = cpu.reg.a;
                cpu.reg.a = 0x44;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                cpu.reg.a = 0x01;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::OPACITY] = cpu.reg.a;
                crate::ext::addback(cpu);
                pc = 3;
            }
            2 => {
                cpu.reg.a = cpu.mem[sym::gatebot];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::YCO] = cpu.reg.a;
                cpu.reg.a = 0x43;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                cpu.reg.a = 0x02;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::OPACITY] = cpu.reg.a;
                crate::ext::addback(cpu);
                pc = 3;
            }
            3 => {
                cpu.reg.a = 0x02;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::OPACITY] = cpu.reg.a;
                cpu.reg.a = 0x37;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                cpu.reg.a = cpu.mem[sym::gatebot];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x0c_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                pc = 4;
            }
            4 => {
                cpu.mem[sym::YCO] = cpu.reg.a;
                let _o: u8 = 0xc0;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a >= 0xc0 {
                    pc = 11;
                } else {
                    pc = 5;
                }
            }
            5 => {
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x07_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if !cpu.flags.c {
                    pc = 8;
                } else {
                    pc = 6;
                }
            }
            6 => {
                let _o: u8 = cpu.mem[sym::blockthr];
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a < cpu.mem[sym::blockthr] {
                    pc = 8;
                } else {
                    pc = 7;
                }
            }
            7 => {
                crate::ext::addback(cpu);
                cpu.reg.a = cpu.mem[sym::YCO];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!0x08_u8) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    pc = 4;
                } else {
                    pc = 8;
                }
            }
            8 => {
                cpu.reg.a = cpu.mem[sym::YCO];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = true;
                let _r = (cpu.reg.a as u16) + (!cpu.mem[sym::blockthr]) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.flags.c = false;
                let _r = (cpu.reg.a as u16) + (0x01) as u16 + (cpu.flags.c as u16);
                cpu.reg.a = _r as u8;
                cpu.flags.c = (_r >> 8) != 0;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.flags.z {
                    pc = 11;
                } else {
                    pc = 9;
                }
            }
            9 => {
                let _o: u8 = 0x09;
                cpu.flags.c = cpu.reg.a >= _o;
                cpu.flags.z = cpu.reg.a == _o;
                cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.a >= 0x09 {
                    pc = 11;
                } else {
                    pc = 10;
                }
            }
            10 => {
                cpu.reg.y = cpu.reg.a;
                cpu.flags.z = cpu.reg.y == 0;
                cpu.flags.n = (cpu.reg.y >> 7) != 0;
                cpu.reg.a = cpu.mem[(sym::gate8b - 1 + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::IMAGE] = cpu.reg.a;
                crate::ext::addback(cpu);
                pc = 11;
            }
            11 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn restorebot(cpu: &mut Cpu) {
    cpu.reg.x = 0x04;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.mem[sym::IMAGE] = cpu.mem[(sym::pieceb + cpu.reg.x as usize) & 0xffff];
    cpu.reg.a = cpu.mem[(sym::pieceby + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = false;
    let _r = (cpu.reg.a as u16) + cpu.mem[sym::Ay] as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = 0x02;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    checkc(cpu);
    if !cpu.flags.c {
        drawa(cpu);
        return;
    }
    dodrawc(cpu);
    drawa(cpu);
    return;
}

pub fn drawobjx(cpu: &mut Cpu) {
    loadobj(cpu);
    if cpu.reg.a == 0x00 {
        crate::ext::DrawKid(cpu);
        return;
    }
    if cpu.reg.a != 0x04 {
        if cpu.reg.a != 0x01 {
            if cpu.reg.a != 0x80 {
                if cpu.reg.a == 0x03 {
                    crate::ext::DrawSword(cpu);
                    return;
                }
                if cpu.reg.a != 0x05 {
                    if cpu.reg.a != 0x02 {
                        return;
                    }
                    crate::ext::DrawGuard(cpu);
                    return;
                }
                crate::ext::DrawSword(cpu);
                return;
            }
            DrawFF(cpu);
            return;
        }
        crate::ext::DrawShad(cpu);
        return;
    }
    crate::ext::DrawKid(cpu);
    return;
}

pub fn DrawFF(cpu: &mut Cpu) {
    cpu.mem[sym::FCharFace] = 0xff;
    cpu.mem[sym::FCharImage] = cpu.mem[sym::IMAGE];
    cpu.reg.a = cpu.mem[sym::FCharY];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!0x03_u8) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.x = 0x01;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.mem[sym::IMAGE] = cpu.mem[(sym::maska + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::OPACITY] = 0x00;
    cpu.reg.a = 0x02;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::addmid(cpu);
    cpu.reg.x = cpu.mem[sym::FCharImage];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.mem[sym::IMAGE] = cpu.mem[(sym::loosea + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::OPACITY] = 0x01;
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::addmid(cpu);
    cpu.reg.x = cpu.mem[sym::FCharImage];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.mem[sym::IMAGE] = cpu.mem[(sym::loosed + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::YCO] = cpu.mem[sym::FCharY];
    cpu.mem[sym::OPACITY] = 0x02;
    cpu.reg.a = 0x02;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::addmid(cpu);
    cpu.mem[sym::XCO] = (cpu.mem[sym::FCharX]).wrapping_add(0x04);
    cpu.reg.a = cpu.mem[sym::FCharY];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.flags.c = true;
    let _r = (cpu.reg.a as u16) + (!0x04_u8) as u16 + (cpu.flags.c as u16);
    cpu.reg.a = _r as u8;
    cpu.flags.c = (_r >> 8) != 0;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.mem[sym::IMAGE] = 0x1b;
    cpu.mem[sym::OPACITY] = 0x01;
    cpu.reg.a = 0x02;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::addmid(cpu);
    return;
}

pub fn getobjid(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::SCRNUM];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        crate::ext::GOnull(cpu);
        return;
    }
    cpu.reg.a = cpu.mem[sym::inbuilder];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        getobjbldr(cpu);
        return;
    }
    cpu.mem[sym::state] = cpu.mem[((cpu.mem[sym::BlueSpec] as usize | (cpu.mem[sym::BlueSpec + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueType] as usize | (cpu.mem[sym::BlueType + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a &= 0x1f;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    match cpu.reg.a {
        0x06 => {
            cpu.reg.a = cpu.mem[sym::state];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.x = cpu.reg.a;
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            cpu.reg.a = cpu.mem[(sym::LINKMAP + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.a &= 0x1f;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a < 0x02 {
                cpu.reg.a = 0x06;  // plate up
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                return;
            }
            cpu.reg.a = 0x05;  // plate depressed
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        0x0f => {
            cpu.reg.a = cpu.mem[sym::state];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.x = cpu.reg.a;
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            cpu.reg.a = cpu.mem[(sym::LINKMAP + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.a &= 0x1f;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a < 0x02 {
                cpu.reg.a = 0x0f;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                return;
            }
            cpu.mem[sym::state] = 0x00;
            cpu.reg.a = 0x01;  // depressed upp looks just like floor
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        _ => {}
    }
    return;
}

pub fn getobjbldr(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueType] as usize | (cpu.mem[sym::BlueType + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.reg.a &= 0x1f;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                getinitobj1(cpu);
                if cpu.flags.c {
                    pc = 8;
                } else {
                    pc = 1;
                }
            }
            1 => {
                cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueSpec] as usize | (cpu.mem[sym::BlueSpec + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                pc = 8;
            }
            2 => {
                cpu.mem[sym::state] = cpu.reg.a;
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                return;
            }
            3 => {
                pc = 4;
            }
            4 => {
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::switches] = cpu.reg.a;
                cpu.reg.x = cpu.mem[sym::sortX];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 5;
            }
            5 => {
                let _o: u8 = 0x01;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x == 0x01 {
                    pc = 9;
                } else {
                    pc = 6;
                }
            }
            6 => {
                cpu.mem[sym::xsave] = cpu.reg.x;
                compare(cpu);
                cpu.reg.x = cpu.mem[sym::xsave];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                if !cpu.flags.c {
                    pc = 8;
                } else {
                    pc = 7;
                }
            }
            7 => {
                cpu.reg.a = cpu.mem[(sym::sortX + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[(sym::sortX - 1 + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::sortX + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::sortX - 1 + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                pc = 8;
            }
            8 => {
                let _v = cpu.reg.x.wrapping_sub(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if cpu.reg.x != 0x00 {
                    pc = 5;
                } else {
                    pc = 9;
                }
            }
            9 => {
                cpu.reg.a = cpu.mem[sym::switches];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    pc = 4;
                } else {
                    pc = 10;
                }
            }
            10 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

#[doc(alias = ":newpass")]
pub fn sortlist(cpu: &mut Cpu) {
    let mut pc: u32 = 0;
    loop {
        match pc {
            0 => {
                cpu.reg.a = 0x00;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[sym::switches] = cpu.reg.a;
                cpu.reg.x = cpu.mem[sym::sortX];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                pc = 1;
            }
            1 => {
                let _o: u8 = 0x01;
                cpu.flags.c = cpu.reg.x >= _o;
                cpu.flags.z = cpu.reg.x == _o;
                cpu.flags.n = (cpu.reg.x.wrapping_sub(_o) >> 7) != 0;
                if cpu.reg.x == 0x01 {
                    pc = 5;
                } else {
                    pc = 2;
                }
            }
            2 => {
                cpu.mem[sym::xsave] = cpu.reg.x;
                compare(cpu);
                cpu.reg.x = cpu.mem[sym::xsave];
                cpu.flags.z = cpu.reg.x == 0;
                cpu.flags.n = (cpu.reg.x >> 7) != 0;
                if !cpu.flags.c {
                    pc = 4;
                } else {
                    pc = 3;
                }
            }
            3 => {
                cpu.reg.a = cpu.mem[(sym::sortX + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.stack.push(cpu.reg.a);
                cpu.reg.a = cpu.mem[(sym::sortX - 1 + cpu.reg.x as usize) & 0xffff];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::sortX + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                cpu.reg.a = cpu.stack.pop().expect("pla on empty stack");
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                cpu.mem[(sym::sortX - 1 + cpu.reg.x as usize) & 0xffff] = cpu.reg.a;
                pc = 4;
            }
            4 => {
                let _v = cpu.reg.x.wrapping_sub(1);
                cpu.reg.x = _v;
                cpu.flags.z = _v == 0;
                cpu.flags.n = (_v >> 7) != 0;
                if cpu.reg.x != 0x00 {
                    pc = 1;
                } else {
                    pc = 5;
                }
            }
            5 => {
                cpu.reg.a = cpu.mem[sym::switches];
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                if cpu.reg.a != 0x00 {
                    sortlist(cpu);
                    return;
                } else {
                    pc = 6;
                }
            }
            6 => {
                return;
            }
            _ => unreachable!(),
        }
    }
}

pub fn compare(cpu: &mut Cpu) {
    cpu.mem[sym::obj1] = cpu.mem[(sym::sortX + cpu.reg.x as usize) & 0xffff];
    cpu.mem[sym::obj2] = cpu.mem[(sym::sortX - 1 + cpu.reg.x as usize) & 0xffff];
    cpu.reg.x = cpu.mem[sym::obj1];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.y = cpu.mem[sym::obj2];
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::objTYP + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x01 {
        return;
    }
    cpu.reg.a = cpu.mem[(sym::objY + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    let _o: u8 = cpu.mem[(sym::objY + cpu.reg.y as usize) & 0xffff];
    cpu.flags.c = cpu.reg.a >= _o;
    cpu.flags.z = cpu.reg.a == _o;
    cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
    if cpu.reg.a != cpu.mem[(sym::objY + cpu.reg.y as usize) & 0xffff] {
        if !cpu.flags.c {
            return;
        }
        if cpu.flags.c {
            return;
        }
    }
    return;
}

pub fn GETINITOBJ(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueType] as usize | (cpu.mem[sym::BlueType + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a &= 0x1f;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x04 {
        if cpu.reg.a == 0x0b {
            cpu.reg.a = 0x00;  // loose floor
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        let _o: u8 = 0x0a;
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.a == 0x0a {
            cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueSpec] as usize | (cpu.mem[sym::BlueSpec + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        if !cpu.flags.z {
            return;
        }
    }
    cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueSpec] as usize | (cpu.mem[sym::BlueSpec + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = cpu.reg.a;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::initsettings - 1 + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn getinitobj1(cpu: &mut Cpu) {
    if cpu.reg.a != 0x04 {
        if cpu.reg.a == 0x0b {
            cpu.reg.a = 0x00;  // loose floor
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        let _o: u8 = 0x0a;
        cpu.flags.c = cpu.reg.a >= _o;
        cpu.flags.z = cpu.reg.a == _o;
        cpu.flags.n = (cpu.reg.a.wrapping_sub(_o) >> 7) != 0;
        if cpu.reg.a == 0x0a {
            cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueSpec] as usize | (cpu.mem[sym::BlueSpec + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.flags.c = (cpu.reg.a >> 7) != 0;
            cpu.reg.a = cpu.reg.a.wrapping_shl(1);
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        if !cpu.flags.z {
            return;
        }
    }
    cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueSpec] as usize | (cpu.mem[sym::BlueSpec + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.x = cpu.reg.a;
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    cpu.reg.a = cpu.mem[(sym::initsettings - 1 + cpu.reg.x as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn metbufs3(cpu: &mut Cpu) {
    mbsub(cpu);
    let _v = cpu.reg.y.wrapping_add(1);
    cpu.reg.y = _v;
    cpu.flags.z = _v == 0;
    cpu.flags.n = (_v >> 7) != 0;
    mbsub(cpu);
    let _v = cpu.reg.y.wrapping_add(1);
    cpu.reg.y = _v;
    cpu.flags.z = _v == 0;
    cpu.flags.n = (_v >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::redbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::floorbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::halfbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::fredbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::wipebuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn metbufs2(cpu: &mut Cpu) {
    mbsub(cpu);
    let _v = cpu.reg.y.wrapping_add(1);
    cpu.reg.y = _v;
    cpu.flags.z = _v == 0;
    cpu.flags.n = (_v >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::redbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::floorbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::halfbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::fredbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::wipebuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn mbsub(cpu: &mut Cpu) {
    cpu.reg.a |= cpu.mem[(sym::redbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::floorbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::halfbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::fredbuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a |= cpu.mem[(sym::wipebuf + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn getobjid1(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::inbuilder];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        getobjbldr(cpu);
        return;
    }
    cpu.mem[sym::state] = cpu.mem[((cpu.mem[sym::BlueSpec] as usize | (cpu.mem[sym::BlueSpec + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.reg.a = cpu.mem[((cpu.mem[sym::BlueType] as usize | (cpu.mem[sym::BlueType + 1] as usize) << 8) + cpu.reg.y as usize) & 0xffff];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.reg.a &= 0x1f;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    match cpu.reg.a {
        0x06 => {
            cpu.reg.a = cpu.mem[sym::state];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.x = cpu.reg.a;
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            cpu.reg.a = cpu.mem[(sym::LINKMAP + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.a &= 0x1f;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a < 0x02 {
                cpu.reg.a = 0x06;  // plate up
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                return;
            }
            cpu.reg.a = 0x05;  // plate depressed
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        0x0f => {
            cpu.reg.a = cpu.mem[sym::state];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.x = cpu.reg.a;
            cpu.flags.z = cpu.reg.x == 0;
            cpu.flags.n = (cpu.reg.x >> 7) != 0;
            cpu.reg.a = cpu.mem[(sym::LINKMAP + cpu.reg.x as usize) & 0xffff];
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            cpu.reg.a &= 0x1f;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            if cpu.reg.a < 0x02 {
                cpu.reg.a = 0x0f;
                cpu.flags.z = cpu.reg.a == 0;
                cpu.flags.n = (cpu.reg.a >> 7) != 0;
                return;
            }
            cpu.mem[sym::state] = 0x00;
            cpu.reg.a = 0x01;  // depressed upp looks just like floor
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
            return;
        }
        _ => {}
    }
    return;
}

pub fn drawtorchb(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::inbuilder];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        return;
    }
    cpu.reg.a = cpu.mem[sym::blockxco];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a == 0x00 {
        return;
    }
    cpu.mem[sym::XCO] = cpu.reg.a;
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.x = cpu.mem[sym::spreced];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    crate::ext::setupflame(cpu);
    crate::ext::addback(cpu);
    return;
}

pub fn _3acont1(cpu: &mut Cpu) {
    cpu.mem[sym::IMAGE] = cpu.reg.a;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = cpu.mem[sym::Dy];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    add(cpu);
    cpu.reg.a = 0xff;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    return;
}

pub fn drawflaska(cpu: &mut Cpu) {
    cpu.reg.a = cpu.mem[sym::inbuilder];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    if cpu.reg.a != 0x00 {
        return;
    }
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.reg.a = cpu.mem[sym::Ay];
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::YCO] = cpu.reg.a;
    cpu.reg.x = cpu.mem[sym::state];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    crate::ext::setupflask(cpu);
    cpu.reg.a = 0x01;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    crate::ext::addmidezo(cpu);
    return;
}

pub fn _3asub(cpu: &mut Cpu) {
    cpu.mem[sym::IMAGE] = 0x11;
    cpu.mem[sym::XCO] = cpu.mem[sym::blockxco];
    cpu.mem[sym::YCO] = cpu.mem[sym::Ay];
    cpu.reg.x = cpu.mem[sym::objid];
    cpu.flags.z = cpu.reg.x == 0;
    cpu.flags.n = (cpu.reg.x >> 7) != 0;
    if cpu.reg.x == 0x05 {
        let _v = cpu.mem[sym::YCO].wrapping_add(1);
        cpu.mem[sym::YCO] = _v;
        cpu.flags.z = _v == 0;
        cpu.flags.n = (_v >> 7) != 0;
    }
    cpu.reg.a = 0x00;
    cpu.flags.z = cpu.reg.a == 0;
    cpu.flags.n = (cpu.reg.a >> 7) != 0;
    cpu.mem[sym::OPACITY] = cpu.reg.a;
    add(cpu);
    return;
}

pub fn getloosey(cpu: &mut Cpu) {
    cpu.reg.y = cpu.mem[sym::inbuilder];
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    if cpu.reg.y == 0x00 {
        cpu.reg.y = cpu.reg.a;
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
        if (cpu.reg.y as i8) >= 0 {
            return;
        }
        cpu.reg.a &= 0x7f;
        cpu.flags.z = cpu.reg.a == 0;
        cpu.flags.n = (cpu.reg.a >> 7) != 0;
        if cpu.reg.a >= 0x0b {
            cpu.reg.a = 0x01;
            cpu.flags.z = cpu.reg.a == 0;
            cpu.flags.n = (cpu.reg.a >> 7) != 0;
        }
        cpu.reg.y = cpu.reg.a;
        cpu.flags.z = cpu.reg.y == 0;
        cpu.flags.n = (cpu.reg.y >> 7) != 0;
        return;
    }
    cpu.reg.y = 0x01;
    cpu.flags.z = cpu.reg.y == 0;
    cpu.flags.n = (cpu.reg.y >> 7) != 0;
    return;
}
